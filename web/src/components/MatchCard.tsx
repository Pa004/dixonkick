  ...
  return (
    <div className="overflow-x-auto">
      <table className="mx-auto border-separate border-spacing-0.5 text-center text-[11px]">
        <thead>
          <tr>
            <th scope="col" className="pr-2 text-right font-normal text-neutro-500">Casa\Visita</th>
            {Array.from({ length: size }, (_, i) => (
              <th key={i} scope="col" className="px-1 font-normal text-neutro-400">{i}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {Array.from({ length: size }, (_, r) => (
            <tr key={r}>
              <th scope="row" className="pr-2 text-right font-semibold text-neutro-400">{r}</th>
              {Array.from({ length: size }, (_, c) => (
                <td key={c} className={`min-w-9 rounded px-1.5 py-1 font-semibold tabular-nums ${heatColor(mat[r]?.[c] ?? 0)}`}>
    ...
    <div className="rounded-base bg-neutro-900 shadow-card transition-shadow duration-200 hover:shadow-elevated">
      ...
      >
        <div className="flex items-center justify-between text-xs text-neutro-400">
          <span className="uppercase tracking-wide">{dateStr} · {timeStr}</span>
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex flex-1 items-center gap-3">
            ...
            <span className="truncate font-display text-sm font-semibold text-neutro-100">{fixture.home}</span>
          </div>
          <span className="px-1 text-xs font-bold text-neutro-400">vs</span>
          <div className="flex flex-1 items-center justify-end gap-3 text-right">
            <span className="truncate font-display text-sm font-semibold text-neutro-100">{fixture.away}</span>
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-base bg-neutro-800 font-display text-xs font-bold text-neutro-200">
        ...
        {pred ? (
          <div className="flex flex-col gap-2">
            <ProbabilityBar home={pred.probabilities.home} draw={pred.probabilities.draw} away={pred.probabilities.away} />
            <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
              <div className="flex gap-3 text-neutro-400">
                <span>Marcador: <b className="font-display tabular-nums text-neutro-100">{pred.scoreline.home}-{pred.scoreline.away}</b></span>
                <span>Favorito: <b className="text-neutro-100">{PICK_LABEL[pred.pick]}</b></span>
                <span>Over 2.5: <b className="font-display tabular-nums text-neutro-100">{Math.round(pred.over_25 * 100)}%</b></span>
              </div>
      ...
      {open && pred && (
        <div className="border-t border-neutro-800/60 p-5 pt-4">
          <div className="mb-3 flex flex-wrap gap-x-6 gap-y-1 text-xs text-neutro-400">
            <div>
              Goles esperados:{" "}
              <b className="font-display tabular-nums text-neutro-100">{pred.expected_goals.home.toFixed(2)}</b> vs{" "}
              <b className="font-display tabular-nums text-neutro-100">{pred.expected_goals.away.toFixed(2)}</b>
            </div>
            <div>BTTS Sí: <b className="font-display tabular-nums text-neutro-100">{Math.round(pred.btts_yes * 100)}%</b></div>
            <div>BTTS No: <b className="font-display tabular-nums text-neutro-100">{Math.round(pred.btts_no * 100)}%</b></div>
